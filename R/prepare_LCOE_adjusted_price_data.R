...
#' We adjust the price by calculating the cost changes for each ald_business_unit and
...
prepare_lcoe_adjusted_price_data_weo <- function(input_data,
  ...
  implied_price <- prices_with_lcoe %>%
    ...
    dplyr::select(
      c(
        ...
        .data$ald_business_unit, .data$unit, .data$implied_price, .data$absolute_npm
      )
    )
  ...
  implied_price_over_time <- prices_with_lcoe %>%
    dplyr::inner_join(
      implied_price,
      by = c("source", "scenario", "scenario_geography", "sector", "ald_business_unit", "unit")
    )
...
prepare_lcoe_adjusted_price_data_oxford2021 <- function(input_data_lcoe_oxford,
  ...
  oil_cap <- data %>%
    dplyr::filter(.data$ald_business_unit == "GasCap") %>%
    dplyr::mutate(ald_business_unit = dplyr::if_else(.data$ald_business_unit == "GasCap", "OilCap", .data$ald_business_unit))

  renewables_cap <- data %>%
    dplyr::filter(.data$ald_business_unit == "RenewablesCap") %>%
    dplyr::group_by(.data$scenario, .data$scenario_geography, .data$sector, .data$year, .data$ald_business_unit) %>%
    dplyr::mutate(price = mean(.data$price)) %>%
    unique()

  data <- data %>% dplyr::filter(!.data$ald_business_unit %in% c("RenewablesCap"))
  ...
  data <- data %>%
    dplyr::group_by(
      .data$scenario, .data$scenario_geography, .data$sector,
      .data$ald_business_unit, .data$unit
    ) %>%
    dplyr::arrange(
      .data$scenario, .data$scenario_geography, .data$sector,
      .data$ald_business_unit, .data$unit, .data$year
    ) %>%
    dplyr::mutate(cost_factor = dplyr::first(.data$price) / .data$price) %>%
    dplyr::ungroup()
  ...
  implied_price_over_time <- data %>%
    dplyr::inner_join(
      implied_price,
      by = c("scenario", "scenario_geography", "sector", "ald_business_unit", "unit")
    )
    ...
    for (year in start:end) {
      for (technology in technologies) {
        for (scenario in scenarios) {
          new_row <- data.frame(sector = sector, scenario_geography = scenario_geography, year = year, price = NA, ald_business_unit = technology, unit = unit, scenario = scenario, indicator = indicator, stringsAsFactors = FALSE)
  ...
  tech <- unique(prices_adjusted$ald_business_unit)
