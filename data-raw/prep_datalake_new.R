...
make_eikon_db <- function() {
  get_old_eikon_data <- function() {
    ...
    # )
    path_db_eikon_data <- fs::path("data-raw", "2021Q2")
  ...
  # load recent data
  eikon_new <-
    readr::read_csv("data-raw/eikon_data.csv", na = c("")) %>%
    dplyr::mutate(
...
make_asset_impact_db <- function() {
  ...
  asset_impact_data <-
    readxl::read_excel(fs::path("data-raw", "AR-Company-Indicators.xlsx"),
  ...
  company_activities <-
    read_asset_resolution(fs::path("data-raw", "AR-Company-Indicators.xlsx"),
...
make_ids_db <- function(DB_asset_impact, DB_assets_eikon) {
  ...
  # )
  path_db_analysis_inputs <- fs::path("data-raw")
...
make_ownership_tree_db <- function() {
  ...
  ownership_tree <- readr::read_csv(
    file.path("data-raw", # path_db_datastore,
...
get_additional_isins <- function(DB_ids) {
  new_ar_id_and_isins <-
    readxl::read_excel("data-raw/DBs/CDI  ISINs.xlsx") %>%
